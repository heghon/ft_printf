...
/*   Updated: 2019/05/06 17:45:29 by bmenant          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_data					*organize_the_flags(t_data *data)
{
	...
	if (data->t[1] || data->t[3])
		data->t[13] = 1;
	data->t[11] = data->t[10] + data->t[13] + (2 * data->t[4]);
	if (data->t[5] > data->t[11] && data->t[2])
		data->t[11] = data->t[5];
	if (data->t[6] >= data->t[10])
		data->t[11] = data->t[6] + data->t[13] + (2 * data->t[4]);
...
static char						*stock_the_number(unsigned long long int nbr)
{
	...
	while (nbr)
	{
		str[--i] = ((nbr % 16) > 9 ? (nbr % 16) - 10 + 'A' : (nbr % 16) + '0');
		nbr /= 16;
	}
	return (str);
}
...
t_data							*print_bx(t_data *data)
{
	...
	if (!(data->t[6] == 0 && x == 0))
	{
		if (data->t[4] && x && ++data->len && ++data->len)
			write(1, "0X", 2);
		...
		write(1, str, (int)ft_strlen(str));
			...
			&& data->t[5] != 0)
	{
		data->t[4] ? write(1, "   ", 3) : write(1, " ", 1);
		data->len += (data->t[4] ? 3 : 1);
	}
